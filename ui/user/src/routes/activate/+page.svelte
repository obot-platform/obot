<script lang="ts">
	...
	onMount(async () => {
		...
		const setupToken = fragment.get('token');
		// Remove the bearer secret before any navigation, analytics, or copied browser URL can
		// retain it. URL fragments are not sent in HTTP requests in the first place.
