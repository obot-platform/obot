...
>
	<div class="paper p-4">
		<div class="flex flex-col gap-6">
			...
			<div class="flex flex-col gap-2">
				...
				<label class="bg-base-200 flex items-start gap-3 rounded-lg border p-3">
	...
	<div class="paper flex flex-col gap-2 p-4">
		...
		>
			{#if filteredServers.length === 0}
				<div class="text-base-content/40 flex items-center justify-center py-8 text-sm">
						...
						<div class="flex w-full items-center gap-3 overflow-hidden">
							<div class="shrink-0">
								{#if server.manifest.icon}
									...
									/>
								{:else}
									<Server class="text-base-content/40 size-6" />
								{/if}
							</div>
							<div class="flex min-w-0 grow flex-col">
								<p class="truncate text-sm">{getServerDisplayName(server)}</p>
								{#if server.manifest.description}
									<span class="text-base-content/40 line-clamp-1 text-xs">
	...
	<div
		class="bg-base-200 dark:bg-base-100 dark:text-base-content/40 sticky bottom-0 left-0 flex w-full justify-end gap-2 py-4 text-gray-400"
		out:fly={{ x: -100, duration }}
		in:fly={{ x: -100 }}
	>
