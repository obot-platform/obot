<script lang="ts">
	...
	import type { VMcpRepository } from '$lib/services/admin/types';
	...
	interface Props {
		...
		onEdit: (repository: VMcpRepository) => void;
		onDelete: (repositories: VMcpRepository[]) => void;
	...
	let vmcpRepositoriesTableData = $derived(
		(query
			? vmcpRepositories.filter(
					(d) =>
...
<div class="flex min-h-full flex-col" in:fade={{ duration: PAGE_TRANSITION_DURATION }}>
	...
	{#if vmcpRepositories.length > 0}
		<Table
			data={vmcpRepositoriesTableData}
