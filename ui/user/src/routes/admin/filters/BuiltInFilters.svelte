	...
	<div class="p-4 md:p-0">
		...
		>
			<p class="flex flex-col gap-1 text-left">
				...
				<span class="text-sm font-light text-base-content/40 line-clamp-3">
					{stripMarkdownToText(filteredBuiltInFiltersData[0].manifest.description)}
				</span>
			</p>
			<StepForward class="size-4 shrink-0 text-base-content/40" />
		</button>
	</div>
{/if}

<div class="text-base-content/40 text-sm font-light mt-4 text-center italic">More Coming Soon!</div>
