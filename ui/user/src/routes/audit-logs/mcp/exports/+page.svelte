<script lang="ts">
		...
		if (formType === 'export' || formType === 'scheduled') {
			try {
				...
				if (response.provider) {
					...
					showForm = formType;
					goto(`/audit-logs/mcp/exports?form=${formType}`, { replaceState: false });
				} else {
					// No storage provider configured, redirect to storage form first
					showForm = 'storage';
					goto(`/audit-logs/mcp/exports?form=storage&next=${formType}`, {
						replaceState: false
					});
				}
			} catch (error) {
	...
	function closeForm() {
		showForm = null;
		goto('/audit-logs/mcp/exports', { replaceState: false });
	}
	...
	async function handleFormSuccess(item?: AuditLogExport | ScheduledAuditLogExport) {
		...
		await goto('/audit-logs/mcp/exports', { replaceState: false });
