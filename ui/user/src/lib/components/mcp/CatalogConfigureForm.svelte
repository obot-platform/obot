<script lang="ts">
	...
	function missingRequiredFields(formAny: LaunchFormData | CompositeLaunchFormData) {
		...
		return [...envs, ...headers].some(
			(field) => field.required && !field.secretBinding && !field.value
		);
	}
		...
		if (isCompositeForm(formAny)) {
			for (const [compId, comp] of Object.entries(formAny.componentConfigs || {})) {
				...
				for (const f of comp.envs ?? []) {
					if (f.required && !f.secretBinding && !f.value)
											...
											<div class="flex flex-col gap-1">
												...
												{#if env.secretBinding}
													<span class="text-on-surface1 text-sm italic">
														Bound to K8s Secret: {env.secretBinding.secretName}/{env.secretBinding
															.secretKey}
													</span>
