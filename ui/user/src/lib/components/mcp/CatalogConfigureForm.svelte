								...
								<span class="flex items-center gap-2">
									<label for={env.key} class={highlightRequired ? 'text-red-500' : ''}>
										...
										{#if env.componentDisplayName}
											<span
												class="ml-1 rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-300"
												>{env.componentDisplayName}</span
											>
										{/if}
										{#if !env.required}
											<span class="text-gray-400 dark:text-gray-600">(optional)</span>
								...
								<span class="flex items-center gap-2">
									<label for={header.key}>
										...
										{#if header.componentDisplayName}
											<span
												class="ml-1 rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-300"
												>{header.componentDisplayName}</span
											>
										{/if}
										{#if !header.required}
											<span class="text-gray-400 dark:text-gray-600">(optional)</span>
