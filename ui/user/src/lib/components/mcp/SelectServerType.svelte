<script lang="ts">
	import { Container, User, Layers, Search, Bot, Server } from 'lucide-svelte';
...
<ResponsiveDialog title="Add MCP Server" class="md:w-lg" bind:this={selectServerTypeDialog}>
	<div class="my-4 flex flex-col gap-4">
		...
		>
			...
			/>
			<div>
				<p class="mb-1 text-sm font-semibold">Add From Official Obot Catalog</p>
				<span class="block text-xs leading-4 text-gray-400 dark:text-gray-600">
					...
					Select existing remote servers or MCP Servers templates from the official Obot catalog to add
					to your registry.
				</span>
			</div>
		</button>
		...
		{#if profile.current?.groups.includes(Group.POWERUSER_PLUS)}
			...
			>
				...
				/>
				<div>
					<p class="mb-1 text-sm font-semibold">Add Obot Hosted Server</p>
					<span class="block text-xs leading-4 text-gray-400 dark:text-gray-600">
						<!-- This option is appropriate for servers designed to handle multiple user connections,
						such as most Streamable HTTP servers. When you create this server, a running instance
						will be deployed and any user with access to this catalog will be able to connect to it. -->
		...
		>
			...
			/>
			<div>
				<p class="mb-1 text-sm font-semibold">Add Remote Server</p>
				<span class="block text-xs leading-4 text-gray-400 dark:text-gray-600">
