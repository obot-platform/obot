...
<script lang="ts">
	...
	onMount(async () => {
		...
		envs = fromMap(envMap).map((e) => ({ ...e, editing: '' }));
	...
	function commitState(state: SaveState) {
		...
		envs = fromMap(state.env).map((e) => ({ ...e, editing: '' }));
		onSave(state.tool);
	}
...
<div class="relative flex flex-col gap-5 rounded-s-lg p-5">
	...
	<div class="flex flex-col">
		<input
			...
			class="bg-surface2 text-xl font-semibold outline-none"
		/>
		<textarea
			use:autoHeight
			bind:value={tool.description}
			placeholder="Enter description (a good one is very helpful)"
			class="bg-surface2 resize-none outline-none"
		></textarea>
	</div>
