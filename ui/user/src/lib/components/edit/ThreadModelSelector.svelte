...
<div class="relative mr-2 md:mr-6 lg:mr-8">
	<button
		class={twMerge(
			'hover:bg-base-300/50 active:bg-base-300/80 flex h-10 items-center gap-3 rounded-full px-2  py-1 text-xs text-base-content/40 md:px-4 lg:px-6',
	...
	>
		...
		{:else if modelsError}
			<div class="text-base-content/40 p-4 text-sm">
				{modelsError}
			</div>
		{:else if availableModels.length === 0}
			<div class="text-base-content/40 p-4 text-sm">No Model Available</div>
		{:else}
