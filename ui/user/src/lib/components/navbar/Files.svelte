						...
						<div class="flex">
							...
							>
								{#if isImage(file.name)}
									<Image class="h-5 w-5 min-w-fit" />
								{:else}
									<FileText class="h-5 w-5 min-w-fit" />
								{/if}

								<Truncate text={file.name} class="ms-2 group-hover:underline" />
							</button>
							<button
								class="invisible ms-2 group-hover:visible"
								onclick={() => {
