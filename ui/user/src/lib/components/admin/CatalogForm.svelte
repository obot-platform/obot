<script lang="ts">
	...
	let selectServerTypeDialog = $state<HTMLDialogElement>();
					...
					<DotDotDot class="button-primary text-sm">
						...
						<div class="default-dialog flex min-w-max flex-col p-2">
							<button
								class="menu-button"
								onclick={() => {
									selectServerTypeDialog?.showModal();
								}}
							>
								Add server
							</button>
...
<dialog
	...
	use:clickOutside={() => selectServerTypeDialog?.close()}
...
>
	<h3 class="default-dialog-title" class:default-dialog-mobile-title={responsive.isMobile}>
		...
		<button
			...
			onclick={() => selectServerTypeDialog?.close()}
			class="icon-button"
		>
	...
	<div class="my-4 flex flex-col gap-4">
		<button
			class="dark:bg-surface2 hover:bg-surface1 dark:hover:bg-surface3 dark:border-surface3 border-surface2 cursor-pointer rounded-md border bg-white px-2 py-4 text-left transition-colors duration-300"
			onclick={() => selectServerType('single')}
		>
			<p class="mb-1 text-center text-sm font-semibold">Single User Server</p>
			<span class="block text-xs leading-4 text-gray-400 dark:text-gray-600">
				...
				designed for multi-user access, such as most studio MCP servers. When a user selects this
				server, a private instance will be created for them.
			</span>
		</button>
		<button
			class="dark:bg-surface2 hover:bg-surface1 dark:hover:bg-surface3 dark:border-surface3 border-surface2 cursor-pointer rounded-md border bg-white px-2 py-4 text-left transition-colors duration-300"
		...
		>
			<p class="mb-1 text-center text-sm font-semibold">Multi-User Server</p>
			<span class="block text-xs leading-4 text-gray-400 dark:text-gray-600">
				This option is appropriate for servers designed to handle multiple user connections, such as
				most Streamable HTTP servers. When you create this server, a running instance will be
		...
		>
			<p class="mb-1 text-center text-sm font-semibold">Remote Server</p>
			<span class="block text-xs leading-4 text-gray-400 dark:text-gray-600">
