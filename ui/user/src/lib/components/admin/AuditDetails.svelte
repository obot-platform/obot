<script lang="ts">
	import { ChevronsLeft, ChevronsRight, LoaderCircle } from 'lucide-svelte';
...
{:then auditLogsResponse}
	...
	{:then usageResponse}
		...
		<div
			class="dark:border-surface3 dark:bg-surface1 mb-1 w-full rounded-md border border-transparent bg-white p-4 shadow-sm"
		>
			{#if graphView === 'calls'}
	...
	{@const auditLogs = (auditLogsResponse?.items ?? []).map((auditLog) => {
	...
	{#if auditLogs.length > 0}
		<Table
			data={auditLogs}
			fields={['createdAt', 'callIdentifier', 'callType', 'mcpServerDisplayName', 'user', 'client']}
			headers={[
	...
	{:else if mcpId || mcpCatalogEntryId}
