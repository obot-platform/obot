<script lang="ts" generics="T extends object">
	...
	let { auditLog, onClose, preRequestContent, additRequestContent, loading }: Props<T> = $props();
	...
	const shouldShowPayload = $derived(
		!!auditLog && (profile?.current?.hasAdminAccess?.() || auditLog.userID === profile.current.id)
	);
			...
			{#if shouldShowPayload}
				{#if auditLog?.webhookStatuses && auditLog.webhookStatuses.length > 0}
					...
					<div class="relative text-white">
						<pre class="default-scrollbar-thin max-h-96 overflow-y-auto p-4"><code class="language-json">{statuses}</code></pre>
...
{#snippet jsonBody(name: string, value: unknown)}
	...
	<div class="relative text-white">
		<pre class="default-scrollbar-thin max-h-96 overflow-y-auto p-4"><code class="language-json">{body}</code></pre>
