...
{#snippet th(content: string, { class: klass = '', minWidth = '0ch' } = {})}
	<th
		class={twMerge(
			'dark:bg-base-100 text-base-content/40 sticky top-0 box-content w-[24ch] truncate text-left text-xs font-medium tracking-wider uppercase',
			klass
		)}
			...
			{#snippet header()}
				<thead>
					<tr bind:this={headerRowElement}>
						<th
							class="dark:bg-base-100 bg-base-200 text-base-content/40 sticky top-0 box-content w-[4ch] px-6 py-3 text-left text-xs font-medium tracking-wider uppercase"
						>
							<div>#</div>
						</th>
				...
				{#each items as item (item.data.id)}
					...
					<tr
						class={twMerge(
							...
							onSelectRow && 'hover:bg-base-100 dark:hover:bg-base-400 cursor-pointer'
						)}
