...
The directory tree below highlights the files required to implement `Hash` in Typescript and package it for `Acorn`.
...
Before adding a Tool to `Acorn`, verify that the Typescript business logic works on your machine.
...
## Adding The `Hash` Tool to `Acorn` {#adding-the-hash-tool-to-otto8-aeed7f15}
