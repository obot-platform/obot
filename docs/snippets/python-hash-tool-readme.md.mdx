...
The directory tree below highlights the files required to implement `Hash` in Python and package it for `Acorn`.
...
Before adding a Tool to `Acorn`, verify that the Python business logic works on your machine.
...
## Adding The `Hash` Tool to `Acorn` {#adding-the-hash-tool-to-otto8-28572835}
