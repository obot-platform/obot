...
This guide walks through the structure and design of the Tool and outlines the packaging requirements for [Acorn](https://docs.otto8.ai/concepts/agents)
...
The directory tree below highlights the files required to implement `Hash` in Go and package it for `Acorn`.
...
The `tool.gpt` file contains [GPTScript Tool Definitions](https://docs.gptscript.ai/tools/gpt-file-reference) which describe a set of Tools that can be used by Agents in `Acorn`.
...
Immediately below the `Preamble` is the `Tool Body`, which tells `Acorn` how to execute the Tool:
...
To oversimplify, when an Agent calls the `Hash` Tool, `Acorn` reads this line and then:
...
The `tool.gpt` file also provides the following metadata for use in `Acorn`:
...
The `main.go` file is the entry point of the `gptscript-go-tool` binary that is executed by `Acorn` when the `Hash` Tool is called.
...
Before adding a Tool to `Acorn`, verify that the Go business logic works on your machine.
...
## Adding The `Hash` Tool to `Acorn` {#adding-the-hash-tool-to-otto8-eb5bb8c4}

Before a Tool can be used by an Agent, an admin must first add the Tool to `Acorn` by performing the steps below:

1. <details>
       <summary>Navigate to the <code>Acorn</code> admin UI in a browser and open the Tools page by clicking the <em>Tools</em> button in the left drawer</summary>
       <div align="left">
