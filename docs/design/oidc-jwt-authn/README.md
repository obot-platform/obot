...
- Reading a configured eligibility claim (`eligible` for the Studio integration; the claim name is configurable for future providers) and refusing calls when the claim is absent or false.
- Reading a configured roles claim (`roles` for the Studio integration) and mapping it to Obot's existing groups: configured admin roles receive Admin without Owner, configured owner roles receive Owner and its inherited capabilities, and other callers remain authenticated users.
...
| Map JWTs to Obot users via the existing identity layer, then derive groups from the configured roles claim                                                 | Same `{ issuer, sub }` mapping the browser-login flow uses. The roles claim is read per call and mapped to Obot's existing groups. Admin and Owner mappings remain distinct so owner-only credential operations cannot be performed by an admin claim. No new authz rules are needed.        |
| Trust the configured eligibility claim; do not call the issuer per request                                                                                 | The issuer (Studio) is responsible for emitting `eligible: false` on JWTs minted for ineligible users. Obot's per-call check is the second layer. JWT TTL bounds the staleness window. No Obot→issuer callback required in v1.                                                               |
...
| JWT authenticator (`pkg/oidcjwt/authenticator.go`) | Implements `authenticator.Request`. Inspects the `Authorization: Bearer …` header; if present and valid, resolves or creates the Obot user via the existing identity layer, checks the eligibility claim, then maps configured owner roles to Owner, configured admin roles to Admin without Owner, and all other roles to Authenticated. Owner mapping takes precedence when a claim matches both lists. |
| Configuration (`pkg/oidcjwt/config.go`)            | Typed config populated from env / chart values. Holds issuer URL, audience, eligibility-claim name, roles-claim name, admin-role mappings, and owner-role mappings. Defaults are `admin` and `owner`, respectively.                                                                                                                                                                                       |
...
| `chart/values.yaml`            | Add the audience, eligibility-claim, roles-claim, admin-role, and owner-role settings under the existing top-level `config:` map. The chart already renders `config:` through a secret and `envFrom`. |
...
2. **Check the eligibility claim.** Configured name (default `eligible`). If absent or falsy, return a real error (401).
3. **Derive groups from the roles claim.** The configured claim name defaults to `roles`. An owner-role match (default `["owner"]`) yields `types.RoleOwner.Groups()`. Otherwise, an admin-role match (default `["admin"]`) yields `types.RoleAdmin.Groups()` with `types.GroupOwner` removed. Otherwise the caller receives only `types.GroupAuthenticated`. Owner precedence keeps overlapping custom mappings deterministic.
...
The claim name is configured (default `eligible`). Recognized shapes:
...
The claim name is configured (default `roles`). Expected shape: array of role strings. The authenticator checks the owner-mapping list before the admin-mapping list, then falls back to Authenticated. Unrecognized roles are ignored — they do not deny the call (the eligibility claim is the deny gate).
...
| `OBOT_GENERIC_OAUTH_AUTH_PROVIDER_ELIGIBILITY_CLAIM_NAME` (new) | The claim name read for eligibility gating. Default `eligible`.                                        |
| `OBOT_GENERIC_OAUTH_AUTH_PROVIDER_ROLES_CLAIM_NAME` (new)       | The claim name read for group mapping. Default `roles`.                                                |
| `OBOT_GENERIC_OAUTH_AUTH_PROVIDER_ADMIN_ROLES`                  | Comma-separated role values that receive Obot admin capabilities without Owner. Default `admin`.       |
| `OBOT_GENERIC_OAUTH_AUTH_PROVIDER_OWNER_ROLES`                  | Comma-separated role values that receive Obot Owner and its inherited capabilities. Default `owner`.   |
...
| JWT with a value in the owner-mapping list (default `owner`)      | `user.Info` with Owner and its inherited groups; owner-only static OAuth credential operations are authorized.                        |
| JWT with a value only in the admin-mapping list (default `admin`) | `user.Info` with Admin and Authenticated groups, without Owner; admin-scoped paths are allowed and owner-only paths remain forbidden. |
...
| JWT with `eligible: false`                                        | Same — 401.                                                                                                                           |
...
- Unit: subject resolution and group mapping. JWT with `roles: [owner]` → owner groups; `roles: [admin]` → admin without owner; `roles: [domain_contributor]` or empty → authenticated only. Eligibility-claim missing or false → 401. New user record created on first call.
...
- Integration: spin up a test issuer and prove admin access on admin-scoped catalog paths, owner access on the owner-only static OAuth credential path, admin rejection from that owner-only path, and authenticated-only rejection from admin paths.
