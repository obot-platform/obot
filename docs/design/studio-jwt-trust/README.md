...
- Reading the `studio_eligible` claim on user-subject JWTs and refusing calls with absent or empty claims.
...
| Trust the `studio_eligible` claim on user-subject JWTs; do not call Studio per request | Studio refuses to mint a user-subject JWT for an ineligible user; Obot's per-call check of `studio_eligible` is the second layer of defense. JWT TTL (Studio default 60-120s for user-subject) bounds staleness. No Obot→Studio callback required in v1. |
...
| `studio_eligible` claim enforcer | On user-subject contexts, refuses the request if the `studio_eligible` claim is absent or empty. Returns `403 forbidden`. |
...
- `pkg/studio/jwt/` — JWT validation, subject-shape resolution, `studio_eligible` claim check, request-context enrichment.
...
### `studio_eligible` claim enforcement

On user-subject contexts, the request is refused with `403 forbidden` (`studio_eligible_missing`) when:

- The `studio_eligible` claim is absent.
- The `studio_eligible` claim is present but parses to falsy / empty.

On backend-subject contexts, the `studio_eligible` claim is not consulted.
...
| User-subject (with `studio_eligible`) | The corresponding Obot user | The user-scoped endpoint families the same user would reach through browser auth: `/api/mcp-servers/**` (user-scoped), `/api/mcp-server-instances/**`, `/api/mcp-servers/{id}/oauth` (disconnect), and other per-user MCP surfaces |
| User-subject (without `studio_eligible`) | Refused with `403` before reaching any endpoint |
...
| User-subject JWT missing `studio_eligible` | Refused with `403 studio_eligible_missing`. |
...
- Obot does not call Studio in v1. The `studio_eligible` claim is the freshness signal; JWT TTL bounds staleness.
...
2. ~~`studio_eligible` claim shape.~~ Resolved. Boolean; `true` iff the user has any active Studio role. An optional companion claim `studio_roles` (array of role strings) is also emitted for forward compatibility; Obot does not gate on `studio_roles` in v1. This matches Studio's existing OAuth-client eligibility-claim configuration at `src/server/modules/auth/oauth-clients/oauth-clients.schemas.ts:10-37` so browser-flow ID tokens and service-identity JWTs share one claim contract.
