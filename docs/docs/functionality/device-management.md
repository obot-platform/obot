...
Device management gives administrators visibility into the AI clients, MCP servers, skills, and plugins configured on user workstations. It can also audit tool calls made by supported local AI clients.
...
Creating the device configuration, changing agent settings, and managing enrollment keys require administrative or owner access. Auditors can view the Configuration view but cannot make changes.
