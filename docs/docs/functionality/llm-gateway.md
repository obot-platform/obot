...
The Obot LLM Gateway lets you call OpenAI and Anthropic models through Obot using your **Obot credentials** instead of a provider API key. Point any OpenAI- or Anthropic-compatible client — such as [Claude Code](#using-with-claude-code) or [Codex](#using-with-codex) — at the gateway, authenticate with your Obot token, and call models by their native provider names (for example `claude-opus-4.8` or `gpt-5.5`).
...
- **Example request** — a ready-to-run `curl` command, pre-filled with one of your available models and with the API key wired to `obot login --print-token`.
...
3. **The Obot CLI.** Install and set up the `obot` CLI to obtain a token. See [Obot CLI Setup](/installation/cli-setup/).

## Getting your API token

Your "API key" for the gateway is your Obot bearer token. Print one with the CLI:

```bash
obot login --url https://obot.example.com --print-token
```

This logs you in (opening a browser if needed) and prints the token to stdout, so you can capture it directly into an environment variable:

```bash
export OBOT_TOKEN="$(obot login --url https://obot.example.com --print-token)"
```

:::note
The `Logged in to ...` confirmation is printed to stderr, so command substitution captures only the token. Add `--no-expiration` if you want a token that doesn't expire.
:::
...
export OPENAI_API_KEY="$(obot login --url https://obot.example.com --print-token)"
...
```bash
# 1. Point Claude Code at the gateway and authenticate with your Obot token.
export ANTHROPIC_BASE_URL="https://obot.example.com/api/llm-proxy/anthropic"
export ANTHROPIC_API_KEY="$(obot login --url https://obot.example.com --print-token)"
...
`ANTHROPIC_API_KEY` is sent as the `x-api-key` header. If your setup prefers a bearer token, you can set `ANTHROPIC_AUTH_TOKEN` instead (sent as `Authorization: Bearer`). The gateway accepts either.
:::
...
2. Set your token and start Codex:
