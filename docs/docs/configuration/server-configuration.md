...
| `OBOT_SERVER_MODEL_PROXY_URL` | MCP Tester model proxy service base URL, used only when no model provider is configured. An explicitly empty string disables fallback. Custom URLs must use HTTPS; HTTP is allowed only with `OBOT_DEV_MODE=true`. | `https://model-service.obot.ai` |
