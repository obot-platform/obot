...
```bash
docker run -d --name obot -p 8080:8080 -v /var/run/docker.sock:/var/run/docker.sock -e OPENAI_API_KEY=<API KEY> -e OBOT_SERVER_ENABLE_AUTHENTICATION=true ghcr.io/obot-platform/obot:latest
```

Then open your browser to [http://localhost:8080](http://localhost:8080) to access the Obot UI. You can find your bootstrap token in the container logs (`docker logs obot`). This is used for initial setup.
