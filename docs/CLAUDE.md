...
Only `overview.md` sets a `slug`, which is `/`, so it becomes the root page of each version.

A leading number in a filename sets the sidebar order and is stripped from the URL, e.g. `installation/reference-architectures/02-aws-eks.md` is served at `/installation/reference-architectures/aws-eks/`. Keep the number when you write a relative link to the file, because the link points at the file and not at the URL.
