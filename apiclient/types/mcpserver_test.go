...
func TestMapCatalogEntryToServer_StartupTimeout(t *testing.T) {
	t.Run("copies nested startup timeout", func(t *testing.T) {
		...
		if result.StartupTimeoutSeconds != 0 {
			t.Errorf("Expected no top-level startup timeout, got %d", result.StartupTimeoutSeconds)
		}
	})

	t.Run("uses legacy startup timeout when nested is unset", func(t *testing.T) {
		result, err := MapCatalogEntryToServer(MCPServerCatalogEntryManifest{
			Runtime:               RuntimeNPX,
		...
		if result.NPXConfig.StartupTimeoutSeconds != 90 {
			t.Errorf("Expected startup timeout 90, got %d", result.NPXConfig.StartupTimeoutSeconds)
		}
	})

	t.Run("nested startup timeout wins over legacy", func(t *testing.T) {
		result, err := MapCatalogEntryToServer(MCPServerCatalogEntryManifest{
			Runtime:               RuntimeUVX,
			StartupTimeoutSeconds: 90,
			UVXConfig: &UVXRuntimeConfig{
		...
		if result.UVXConfig.StartupTimeoutSeconds != 120 {
			t.Errorf("Expected startup timeout 120, got %d", result.UVXConfig.StartupTimeoutSeconds)
		}
	})
}

func TestMigrateStartupTimeoutSeconds(t *testing.T) {
	t.Run("moves legacy timeout to runtime config", func(t *testing.T) {
		...
		if !manifest.MigrateStartupTimeoutSeconds() {
			t.Fatal("Expected migration to report a change")
		}
		if manifest.StartupTimeoutSeconds != 0 {
			t.Errorf("Expected legacy timeout cleared, got %d", manifest.StartupTimeoutSeconds)
		}
		if manifest.ContainerizedConfig.StartupTimeoutSeconds != 100 {
			t.Errorf("Expected nested timeout 100, got %d", manifest.ContainerizedConfig.StartupTimeoutSeconds)
		}
	})

	t.Run("nested timeout wins", func(t *testing.T) {
		manifest := MCPServerManifest{
			Runtime:               RuntimeNPX,
			StartupTimeoutSeconds: 100,
			NPXConfig: &NPXRuntimeConfig{
				Package:               "test-package",
				StartupTimeoutSeconds: 200,
			},
		}
		if !manifest.MigrateStartupTimeoutSeconds() {
			t.Fatal("Expected migration to report a change")
		}
		if manifest.NPXConfig.StartupTimeoutSeconds != 200 {
			t.Errorf("Expected nested timeout 200, got %d", manifest.NPXConfig.StartupTimeoutSeconds)
		}
		if manifest.StartupTimeoutSeconds != 0 {
			t.Errorf("Expected legacy timeout cleared, got %d", manifest.StartupTimeoutSeconds)
		}
	...
	t.Run("drops invalid runtime legacy timeout", func(t *testing.T) {
		manifest := MCPServerManifest{
