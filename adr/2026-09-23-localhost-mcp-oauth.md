...
Remote configuration can opt into localhost callbacks with an optional callback path. Obot derives the provider callback from the validated originating OAuth request's loopback host and port. The CLI accepts provider callbacks only during an active authorization flow and at explicitly allowed paths. The default is `/oauth/callback`; repeatable `--callback-path` flags replace that default. Generated client configurations include the paths required by the server or vMCP components. The CLI receives an allowed callback and redirects the browser to Obot's hosted callback, preserving the query. Obot exchanges the code using the original redirect URI and retains upstream tokens. This applies to direct connections and vMCP components.
