...
func ValidateCatalogConfigurationConstraints(manifest types.MCPServerManifest, catalog types.MCPServerCatalogEntryManifest) error {
	...
	if catalog.Runtime == types.RuntimeRemote && catalog.RemoteConfig != nil {
		if err := validateCatalogURLTemplateEnvConstraints(manifest.Env, catalog.Env, catalog.RemoteConfig.URLTemplate); err != nil {
			return err
		}
	}
