...
func (k *kubernetesBackend) restartServer(ctx context.Context, server ServerConfig) error {
	...
	for attempt := range maxPatchRetries {
		...
		if desiredImage != "" {
			deployedImage := getDeploymentMCPContainerImage(&deployment)
			if deployedImage != "" && deployedImage != desiredImage {
				if err := k.patchDeploymentMCPContainerImage(ctx, &deployment, desiredImage); err != nil {
...
func getDeploymentMCPContainerImage(deployment *appsv1.Deployment) string {
...
func (k *kubernetesBackend) patchDeploymentMCPContainerImage(ctx context.Context, deployment *appsv1.Deployment, image string) error {
