...
func TestComponentToolOverridesStale(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			resolved:  ComponentUpstream{Missing: true},
		},
		{
			name:      "digest matches",
...
func TestResolveComponentUpstream(t *testing.T) {
	...
	t.Run("catalog entry", func(t *testing.T) {
		resolved, err := ResolveComponentUpstream(t.Context(), c, types.ComponentRef{CatalogEntryID: "component-entry"})
	...
	t.Run("multi-user server in catalog entry form", func(t *testing.T) {
		resolved, err := ResolveComponentUpstream(t.Context(), c, types.ComponentRef{MCPServerID: "shared-server"})
		require.NoError(t, err)
		assert.False(t, resolved.Missing)
	...
	t.Run("missing objects are reported, not errors", func(t *testing.T) {
		for _, ref := range []types.ComponentRef{
			{CatalogEntryID: "deleted-entry"},
			{MCPServerID: "deleted-server"},
			{},
		} {
			resolved, err := ResolveComponentUpstream(t.Context(), c, ref)
			require.NoError(t, err)
...
func TestCompositeReadyRequiresSyncRequest(t *testing.T) {
	requested := v1.MCPServer{
		Name:        "composite",
		Namespace:   system.DefaultNamespace,
		Annotations: map[string]string{v1.MCPServerCompositeSyncRequestedAtAnnotation: "request-2"},
		Generation:  3,
	}

	observed := requested
	observed.Status.ObservedCompositeGeneration = requested.Generation
	observed.Status.ObservedCompositeSyncRequest = "request-1"
	assert.False(t, compositeReady(requested, observed))

	observed.Status.ObservedCompositeSyncRequest = "request-2"
	assert.True(t, compositeReady(requested, observed))
}
