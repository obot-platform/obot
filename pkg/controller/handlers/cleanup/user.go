...
const (
	...
	hostedAgentCleanupRetryInterval  = 10 * time.Second
)
...
func (u *UserCleanup) Cleanup(req router.Request, resp router.Response) error {
	...
	complete, err := cleanupHostedAgents(req, resp, userDelete, userID)
	if err != nil {
		return err
	}
	if !complete {
...
func cleanupHostedAgents(req router.Request, resp router.Response, userDelete *v1.UserDelete, userID string) (bool, error) {
	...
	if len(instances.Items) > 0 {
		...
		resp.RetryAfter(hostedAgentCleanupRetryInterval)
