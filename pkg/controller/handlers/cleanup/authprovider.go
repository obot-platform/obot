...
func (a *AuthProviderCleanup) Cleanup(req router.Request, resp router.Response) error {
	...
	if !checkpoint.DataDeleted {
		...
		log.Infof("Deleted auth provider group data: authProvider=%s namespace=%s groupIDPrefix=%s updatedResources=%v", providerName, providerNamespace, groupIDPrefix, counts)
		return nil
	}
	...
	if len(userIDs) > 0 {
		...
		log.Infof("Processed auth provider cleanup user batch: authProvider=%s namespace=%s groupIDPrefix=%s users=%d lastUserID=%d", providerName, providerNamespace, groupIDPrefix, len(userIDs), checkpoint.LastUserID)
		return nil
	}

	log.Infof("Completed auth provider group cleanup: authProvider=%s namespace=%s groupIDPrefix=%s lastUserID=%d", providerName, providerNamespace, groupIDPrefix, checkpoint.LastUserID)
