...
func TestResolveCompositeSourceRefsAppliesMultiUserServerOutsideCatalog(t *testing.T) {
	server := testMCPServer("shared-server", "default", types.MCPServerManifest{
		Name:            "Shared Server",
		Runtime:         types.RuntimeContainerized,
	...
	// A reference to the wrong kind of object is rejected on create, not on sync.
	assert.Empty(t, errsBySourceURL)
	assert.Len(t, result, 1)
