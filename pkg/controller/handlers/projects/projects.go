...
func (*Handler) CleanupChatbots(req router.Request, _ router.Response) error {
	...
	// Bail out early if the thread is already being deleted
	if thread.DeletionTimestamp != nil && !thread.DeletionTimestamp.IsZero() {
		return nil
	}

	// Get the parent thread
	var parentThread v1.Thread
	if err := req.Get(&parentThread, thread.Namespace, thread.Spec.ParentThreadName); err != nil {
	...
	var (
		...
		err              = req.Get(&threadShare, thread.Namespace, projectShareName)
	)
