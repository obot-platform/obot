...
func New(backend agentbackend.InstanceBackend, credentials CredentialIssuer, serverURL string) *Handler {
...
func (b defaultDesiredBuilder) Build(ctx context.Context, in BuildInput) (agentbackend.DesiredInstance, error) {
	...
	config := agentConfig{
		...
		PublicURL:   publicURL,
		MCPServers:  mcpServerConfigs(b.ServerURL, mcpIDs),
		Answers:     answers,
	}
	if source.URL != "" {
	...
	if in.Client != nil {
		models, err := modelConfigs(ctx, in.Client, in.Namespace, b.ServerURL,
