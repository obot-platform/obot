...
func New(ctx context.Context, config Config) (*Services, error) {
	...
	// A sandbox resolves Obot's address from inside the cluster, so a loopback
	// address -- what a developer running Obot on their own machine has --
	// resolves to the pod itself. The node stands in for that machine, since
	// Obot listens on all of its interfaces. In production Obot runs in the
	// cluster and its configured address already resolves, so this is a no-op.
	...
	if agentBackendKind == "kubernetes" && agentLocalK8sClient != nil {
		...
		if reachable != config.Hostname {
			pkgLog.Infof("hosted agent sandboxes will reach Obot at %s", reachable)
		}
