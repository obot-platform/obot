...
func (c *Client) CreateAPIKey(ctx context.Context, userID uint, name, description string, expiresAt *time.Time, mcpServerIDs []string, canAccessSkills, canAppendAuditLogs bool) (*types.APIKeyCreateResponse, error) {
	...
	apiKey := &types.APIKey{
		...
		CanAppendAuditLogs: canAppendAuditLogs,
		ExpiresAt:          expiresAt,
		CreatedAt:          time.Now(),
