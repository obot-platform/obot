...
func (db *DB) AutoMigrate() (err error) {
	...
	// This must run after AutoMigrate has added the generic audit-event columns.
	if err = migrateIfEntryNotFoundInMigrationsTable(tx, "generic_audit_log_fields_backfill", migrateGenericAuditLogFields); err != nil {
		return fmt.Errorf("failed to backfill generic audit log fields: %w", err)
	}
