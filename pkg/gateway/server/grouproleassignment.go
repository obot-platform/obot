...
func (s *Server) createGroupRoleAssignment(apiContext api.Context) error {
	...
	pkgLog.Infof("Created group role assignment: group=%s role=%d requestedBy=%s", req.GroupName, req.Role, apiContext.User.GetName())
...
func (s *Server) updateGroupRoleAssignment(apiContext api.Context) error {
	...
	pkgLog.Infof("Updated group role assignment: group=%s role=%d requestedBy=%s", groupName, req.Role, apiContext.User.GetName())
...
func (s *Server) deleteGroupRoleAssignment(apiContext api.Context) error {
	...
	pkgLog.Infof("Deleted group role assignment: group=%s requestedBy=%s", groupName, apiContext.User.GetName())
...
func (s *Server) validateRoleForUser(apiContext api.Context, role types2.Role) error {
	...
	if baseRole == types2.RoleOwner {
		if !apiContext.UserIsOwner() {
			pkgLog.Infof("Denied group role assignment request: requestedRole=%d reason=owner_role_requires_owner requestUser=%s", role, apiContext.User.GetName())
	...
	if hasAuditor {
		if !apiContext.UserIsOwner() {
			pkgLog.Infof("Denied group role assignment request: requestedRole=%d reason=auditor_role_requires_owner requestUser=%s", role, apiContext.User.GetName())
	...
	if baseRole != 0 {
		...
		if !isValid {
			pkgLog.Infof("Denied group role assignment request: requestedRole=%d reason=invalid_base_role requestUser=%s", role, apiContext.User.GetName())
