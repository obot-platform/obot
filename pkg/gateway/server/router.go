...
func (s *Server) AddRoutes(mux *server.Server) {
	...
	mux.HandleFunc("GET /api/group-role-assignments/{id}", wrap(s.getGroupRoleAssignment))
	mux.HandleFunc("POST /api/group-role-assignments", wrap(s.createGroupRoleAssignment))
	mux.HandleFunc("PUT /api/group-role-assignments/{id}", wrap(s.updateGroupRoleAssignment))
	mux.HandleFunc("DELETE /api/group-role-assignments/{id}", wrap(s.deleteGroupRoleAssignment))
