...
func (s *Server) tokenRequest(apiContext api.Context) error {
	...
	if reqObj.ServiceName != "" {
		list, err := s.dispatcher.ListConfiguredAuthProviders(apiContext.Context(), reqObj.ServiceNamespace)
		if err != nil {
			return types2.NewErrHttp(http.StatusInternalServerError, err.Error())
		}

		if !slices.Contains(list, reqObj.ServiceName) {
			return types2.NewErrHttp(http.StatusBadRequest, fmt.Sprintf("service %q not found", reqObj.ServiceName))
		}
	}
	...
	if reqObj.ServiceName != "" {
		return apiContext.Write(map[string]any{"token-path": fmt.Sprintf("%s/api/oauth/start/%s/%s/%s", s.baseURL, reqObj.ID, reqObj.ServiceNamespace, reqObj.ServiceName)})
	}
...
func (s *Server) redirectForTokenRequest(apiContext api.Context) error {
	...
	if namespace != "" && name != "" {
		...
		if !slices.Contains(list, name) {
			return types2.NewErrHttp(http.StatusBadRequest, fmt.Sprintf("service %q not found", name))
		}
	}
