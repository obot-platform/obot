...
// ListServers handles GET /registry/v0/servers
func (h *Handler) ListServers(req api.Context) error {
	...
	// Get reverse DNS from base URL (strip /api suffix if present)
	baseURL := strings.TrimSuffix(h.serverURL, "/api")
	reverseDNS, err := ReverseDNSFromURL(baseURL)
	if err != nil {
...
func (h *Handler) collectAccessibleServers(req api.Context, reverseDNS string) ([]types.RegistryServerResponse, error) {
	...
	for _, server := range personalServers {
		...
		if err != nil {
			// Skip servers that can't be converted (e.g., composite)
			continue
		}
...
func paginateServers(servers []types.RegistryServerResponse, cursor string, limit int) types.RegistryServerList {
	...
	if endIdx < len(servers) {
		response.Metadata = &types.RegistryMetadata{
...
// ListServerVersions handles GET /v0/servers/{serverName}/versions
...
// GetServerVersion handles GET /v0/servers/{serverName}/versions/{version}
...
func (h *Handler) findServerByName(req api.Context, serverName string, reverseDNS string) (types.RegistryServerResponse, error) {
	...
	if strings.HasPrefix(serverName, "ms1") {
