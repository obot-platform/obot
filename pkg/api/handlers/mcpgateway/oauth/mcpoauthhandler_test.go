...
func TestStaticOAuthAuthStyle(t *testing.T) {
	tests := []struct {
		name            string
		method          string
		...
		{
			name:            "confidential client basic",
			method:          "client_secret_basic",
			hasClientSecret: true,
			want:            oauth2.AuthStyleInHeader,
		},
		{
			name:            "confidential client post",
			method:          "client_secret_post",
			hasClientSecret: true,
			want:            oauth2.AuthStyleInParams,
		},
		{
			name:            "confidential client unspecified",
			hasClientSecret: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.Equal(t, tt.want, staticOAuthAuthStyle(tt.method, tt.hasClientSecret))
		})
	}
}
