...
func (h *TemplateHandler) CreateProjectTemplate(req api.Context) error {
	...
	// Attempt to get the project snapshot name by name
	var (
		...
		snapshotThread = v1.Thread{
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					v1.ProjectSnapshotRevisionAnnotation: now,
				},
			},
			Spec: v1.ThreadSpec{
	...
	if !snapshotThread.Status.SourceThreadUpgradeAvailable {
	...
	// Set the annotation to trigger an upgrade and return the latest snapshot.
	if snapshotThread.Annotations == nil {
		snapshotThread.Annotations = map[string]string{}
	}
	snapshotThread.Annotations[v1.ThreadUpgradeApprovedAnnotation] = "true"
	snapshotThread.Annotations[v1.ProjectSnapshotRevisionAnnotation] = now
...
func (h *TemplateHandler) CopyTemplate(req api.Context) error {
	...
	newProject := v1.Thread{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				v1.ProjectSnapshotRevisionAnnotation: templateThread.Annotations[v1.ProjectSnapshotRevisionAnnotation],
			},
		},
		Spec: v1.ThreadSpec{
