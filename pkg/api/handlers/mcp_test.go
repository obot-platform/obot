...
type countingMCPStorage struct {
	storage.Client
	listCalls int
	getCalls  int
}

func (c *countingMCPStorage) List(ctx context.Context, list kclient.ObjectList, opts ...kclient.ListOption) error {
	c.listCalls++
	return c.Client.List(ctx, list, opts...)
}

func (c *countingMCPStorage) Get(ctx context.Context, key kclient.ObjectKey, obj kclient.Object, opts ...kclient.GetOption) error {
	c.getCalls++
...
func TestConvertMCPServerCompositeAggregatesOnlySecretBoundMissingConfig(t *testing.T) {
	...
	converted := ConvertMCPServer(server, map[string]string{}, "", "", types.ComponentMCPServer{CatalogEntryID: "entry-bound", Configured: false, MissingRequiredEnvVars: []string{"BOUND_ENV", "USER_ENV"}, MissingRequiredHeaders: []string{"BOUND_HEADER", "USER_HEADER"}, Manifest: types.MCPServerManifest{
		Runtime: types.RuntimeRemote,
		Env: []types.MCPEnv{
	...
	}}, types.ComponentMCPServer{CatalogEntryID: "entry-user", Configured: false, MissingRequiredEnvVars: []string{"SHARED_KEY"}, Manifest: types.MCPServerManifest{
		Env: []types.MCPEnv{{Key: "SHARED_KEY"}},
	}})

	assert.False(t, converted.Configured)
	assert.Len(t, converted.Components, 2)
...
func TestConvertMCPServerCompositeSkipsDisabledAndConfiguredComponents(t *testing.T) {
	...
	converted := ConvertMCPServer(server, nil, "", "", types.ComponentMCPServer{CatalogEntryID: "entry-disabled", Configured: false, MissingRequiredEnvVars: []string{"BOUND_DISABLED"}, Manifest: types.MCPServerManifest{
		Env: []types.MCPEnv{{Key: "BOUND_DISABLED", SecretBinding: &types.MCPSecretBinding{Name: "secret", Key: "env"}}},
	}}, types.ComponentMCPServer{CatalogEntryID: "entry-configured", Configured: true})
...
func TestConvertMCPServerCompositeCountsUnconfiguredMultiUserComponent(t *testing.T) {
	...
	converted := ConvertMCPServer(server, nil, "", "", types.ComponentMCPServer{CatalogEntryID: "entry-configured", Configured: true}, types.ComponentMCPServer{MCPServerID: "shared-server", Configured: false, MissingRequiredHeaders: []string{"BOUND_HEADER"}, Manifest: types.MCPServerManifest{
		Runtime: types.RuntimeRemote,
		RemoteConfig: &types.RemoteRuntimeConfig{
...
func TestConvertMCPServerCompositeSkipsDisabledMultiUserComponent(t *testing.T) {
	...
	converted := ConvertMCPServer(server, nil, "", "", types.ComponentMCPServer{MCPServerID: "shared-server", Configured: false, MissingRequiredHeaders: []string{"BOUND_HEADER"}, Manifest: types.MCPServerManifest{
		Runtime: types.RuntimeRemote,
		RemoteConfig: &types.RemoteRuntimeConfig{
			Headers: []types.MCPHeader{{Key: "BOUND_HEADER", SecretBinding: &types.MCPSecretBinding{Name: "secret", Key: "header"}}},
		},
...
func TestConvertMCPServerRetainsUnresolvedComponent(t *testing.T) {
	server := v1.MCPServer{Spec: v1.MCPServerSpec{Manifest: types.MCPServerManifest{
		Runtime:         types.RuntimeComposite,
		CompositeConfig: &types.CompositeRuntimeConfig{ComponentServers: []types.ComponentServer{{CatalogEntryID: "missing-entry"}}},
	}}}

	converted := ConvertMCPServer(server, nil, "", "", types.ComponentMCPServer{
		CatalogEntryID: "missing-entry",
		Error:          "catalog entry not found",
	})

	require.Len(t, converted.Components, 1)
	assert.Equal(t, "missing-entry", converted.Components[0].CatalogEntryID)
	assert.Equal(t, "catalog entry not found", converted.Components[0].Error)
}

func TestCompositeComponentListResolverLoadsResourcesOnce(t *testing.T) {
	sharedManifest := types.MCPServerManifest{
		Name:            "Shared",
		Runtime:         types.RuntimeRemote,
		MultiUserConfig: &types.MultiUserConfig{},
		RemoteConfig:    &types.RemoteRuntimeConfig{URL: "https://shared.example/mcp"},
	}
	sharedServer := &v1.MCPServer{
		Name:      "shared-server",
		Namespace: system.DefaultNamespace,
		Spec:      v1.MCPServerSpec{Manifest: sharedManifest},
	}

	composites := []v1.MCPServer{
		{
			Name:      "composite-a",
			Namespace: system.DefaultNamespace,
	...
	componentServers := []*v1.MCPServer{
		{
			Name:      "component-a",
			Namespace: system.DefaultNamespace,
			Spec: v1.MCPServerSpec{
				UserID:                    "user-1",
				CompositeName:             composites[0].Name,
				MCPServerCatalogEntryName: "entry-a",
		...
		},
		{
			...
			Spec: v1.MCPServerSpec{
				UserID:                    "user-1",
				CompositeName:             composites[1].Name,
				MCPServerCatalogEntryName: "entry-b",
				Manifest:                  types.MCPServerManifest{Runtime: types.RuntimeNPX},
			},
		},
	}
	componentInstances := []*v1.MCPServerInstance{
		{
			Name:      "instance-a",
			Namespace: system.DefaultNamespace,
			Spec: v1.MCPServerInstanceSpec{
				UserID:          "user-1",
				CompositeName:   composites[0].Name,
				MCPServerName:   sharedServer.Name,
				MultiUserConfig: &types.MultiUserConfig{},
			},
		},
		{
	...
	storageClient := &countingMCPStorage{Client: newFakeStorage(t,
		&composites[0], &composites[1], sharedServer,
		componentServers[0], componentServers[1],
		componentInstances[0], componentInstances[1],
	)}
	req := api.Context{
		Request:       httptest.NewRequest(http.MethodGet, "/api/mcp-servers", nil),
		Storage:       storageClient,
		GatewayClient: newHandlerTestGateway(t),
	}
	resolver := compositeComponentListResolver{}

	for _, composite := range composites {
		components, err := resolver.resolve(req, composite, "")
		require.NoError(t, err)
		require.Len(t, components, 2)
		assert.Contains(t, []string{components[0].ComponentID(), components[1].ComponentID()}, sharedServer.Name)
		for _, component := range components {
			if component.MCPServerID == sharedServer.Name {
				assert.Equal(t, sharedManifest, component.Manifest)
			}
		}
	}

	assert.Equal(t, 2, storageClient.listCalls)
	assert.Zero(t, storageClient.getCalls)
}

func TestResolveCatalogEntryComponentsHydratesManifests(t *testing.T) {
	...
	components, err := resolveCatalogEntryComponents(t.Context(), newFakeStorage(t, componentEntry, sharedServer), compositeEntry)
	require.NoError(t, err)

	require.Len(t, components, 2)
	assert.Equal(t, "GitHub", components[0].Name)
	assert.Equal(t, "https://example.com/gh.svg", components[0].Icon)
	assert.False(t, components[0].Missing)
	assert.True(t, components[0].ToolOverridesStale)
	require.NotNil(t, components[0].Manifest.NPXConfig)
	assert.Equal(t, "@example/github@1.0.0", components[0].Manifest.NPXConfig.Package)

	assert.Equal(t, "shared-server", components[1].MCPServerID)
	assert.Equal(t, "Slack", components[1].Name)
	// A multi-user server's manifest is reported in catalog-entry form.
	assert.Equal(t, sharedServer.Spec.Manifest.ConvertToCatalogEntry(), components[1].Manifest)

	// The stored manifest still carries references only.
	require.NotNil(t, compositeEntry.Spec.Manifest.CompositeConfig)
	assert.Equal(t, []types.CatalogComponentServer{
		{CatalogEntryID: "gh-entry", ToolPrefix: "gh", SourceDigest: "captured-digest"},
		{MCPServerID: "shared-server", ToolPrefix: "slack"},
	}, compositeEntry.Spec.Manifest.CompositeConfig.ComponentServers)
}

func TestResolveCatalogEntryComponentsFallsBackToStampedNameAndIcon(t *testing.T) {
	compositeEntry := v1.MCPServerCatalogEntry{
		Name:      "composite-entry",
		Namespace: system.DefaultNamespace,
		Spec: v1.MCPServerCatalogEntrySpec{
	...
	components, err := resolveCatalogEntryComponents(t.Context(), newFakeStorage(t), compositeEntry)
	require.NoError(t, err)
...
func TestResolveCatalogEntryComponentsSkipsNonComposite(t *testing.T) {
	entry := v1.MCPServerCatalogEntry{
		Name:      "npx-entry",
		Namespace: system.DefaultNamespace,
		Spec: v1.MCPServerCatalogEntrySpec{
	...
	components, err := resolveCatalogEntryComponents(t.Context(), newFakeStorage(t), entry)
	require.NoError(t, err)
...
func TestServerManifestFromCatalogEntryManifestPreservesCompositeAdminState(t *testing.T) {
	entry := types.MCPServerCatalogEntryManifest{
		Name:    "Catalog name",
		Runtime: types.RuntimeComposite,
		CompositeConfig: &types.CompositeCatalogConfig{ComponentServers: []types.CatalogComponentServer{{
			CatalogEntryID: "component",
			ToolOverrides:  []types.ToolOverride{{Name: "new-tool", Enabled: true}},
		}}},
	}
	stored := types.MCPServerManifest{
		Name:      "Admin name",
		Resources: &types.MCPResourceRequirements{Limits: types.MCPResourceRequests{CPU: "1"}},
		CompositeConfig: &types.CompositeRuntimeConfig{ComponentServers: []types.ComponentServer{{
			CatalogEntryID: "component",
			Disabled:       true,
			ToolOverrides:  []types.ToolOverride{{Name: "old-tool", Enabled: true}},
		}}},
	}

	manifest, err := mcp.ServerManifestFromCatalogEntryManifest(true, true, entry, stored)
	require.NoError(t, err)
	assert.Equal(t, "Admin name", manifest.Name)
	assert.Equal(t, stored.Resources, manifest.Resources)
	require.Len(t, manifest.CompositeConfig.ComponentServers, 1)
	assert.True(t, manifest.CompositeConfig.ComponentServers[0].Disabled)
	assert.Equal(t, entry.CompositeConfig.ComponentServers[0].ToolOverrides, manifest.CompositeConfig.ComponentServers[0].ToolOverrides)
}
