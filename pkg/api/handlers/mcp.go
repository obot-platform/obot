...
func (m *MCPHandler) RestartServerDeployment(req api.Context) error {
	...
	if !req.UserIsAdmin() && server.Spec.UserID != req.User.GetUID() {
		// Check workspace permissions for power users
		workspaceID := req.PathValue("workspace_id")
		if workspaceID == "" || workspaceID != server.Spec.PowerUserWorkspaceID {
			return types.NewErrNotFound("MCP server %s not found", server.Name)
		}
	}
...
func (m *MCPHandler) StreamServerLogs(req api.Context) error {
	...
	// Multi-user catalog servers (MCPCatalogID set) are excluded since they serve multiple users.
	userOwnsServer := server.Spec.UserID == req.User.GetUID()
	isMultiUserCatalogServer := server.Spec.MCPCatalogID != ""

	allowAccess := userOwnsServer && !isMultiUserCatalogServer
