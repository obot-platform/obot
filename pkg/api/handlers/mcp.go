...
func addExtractedEnvVars(server *v1.MCPServer) {
	...
	case types.RuntimeUVX:
		if server.Spec.Manifest.UVXConfig != nil {
			...
			for _, v := range extractEnvVars(server.Spec.Manifest.UVXConfig.Command) {
				extracted[v] = struct{}{}
			}

			// Extract variables from args
			for _, arg := range server.Spec.Manifest.UVXConfig.Args {
	...
	case types.RuntimeNPX:
		if server.Spec.Manifest.NPXConfig != nil {
			// Extract variables from args
			for _, arg := range server.Spec.Manifest.NPXConfig.Args {
				for _, v := range extractEnvVars(arg) {
					extracted[v] = struct{}{}
				}
			}
	...
	case types.RuntimeContainerized:
		if server.Spec.Manifest.ContainerizedConfig != nil {
			// Extract variables from command
			for _, v := range extractEnvVars(server.Spec.Manifest.ContainerizedConfig.Command) {
				extracted[v] = struct{}{}
			}
			// Extract variables from args
			for _, arg := range server.Spec.Manifest.ContainerizedConfig.Args {
				for _, v := range extractEnvVars(arg) {
					extracted[v] = struct{}{}
				}
			}
	...
	case types.RuntimeRemote:
		if server.Spec.Manifest.RemoteConfig != nil {
			...
			for _, v := range extractEnvVars(server.Spec.Manifest.RemoteConfig.URL) {
				extracted[v] = struct{}{}
			}
		}
	}
