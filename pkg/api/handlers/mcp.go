...
func (m *MCPHandler) TriggerUpdate(req api.Context) error {
	...
	planner, err := m.catalogUpgradePlanner(req)
	if err != nil {
		return err
	}
...
func (m *MCPHandler) ApplyCatalogUpgrade(req api.Context) error {
	...
	result, err := planner.Apply(req.Context(), server.Name, input)
	if err != nil {
		return err
	}
...
func (m *MCPHandler) catalogUpgradePlanner(req api.Context) (*catalogupgrade.Planner, error) {
	validationOptions, err := ValidationOptionsWithResourceMaximums(req, m.mcpSessionManager)
	if err != nil {
		return nil, err
	}
