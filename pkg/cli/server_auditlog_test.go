...
func TestLocalAgentAuditLogFlags(t *testing.T) {
	for _, tt := range []struct {
		name      string
		env       string
		args      []string
		...
		{
			name: "zero flag",
			args: []string{"--local-agent-audit-log-max-body-bytes=0"},
			want: new(0),
		},
		{
			name: "positive flag",
			args: []string{"--local-agent-audit-log-max-body-bytes=100"},
			want: new(100),
		},
		{
			name: "zero environment",
		...
		{
			name:      "invalid flag",
			args:      []string{"--local-agent-audit-log-max-body-bytes=invalid"},
			wantError: true,
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("OBOT_SERVER_LOCAL_AGENT_AUDIT_LOG_MAX_BODY_BYTES", tt.env)
			t.Setenv("OBOT_SERVER_DISABLE_LOCAL_AGENT_AUDIT_LOG", "true")
			...
			require.NoError(t, err)
			require.Equal(t, tt.want, server.LocalAgentAuditLogMaxBodyBytes)
			require.True(t, server.DisableLocalAgentAuditLog)
		})
	}
}
